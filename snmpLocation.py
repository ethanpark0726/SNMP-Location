...
def createExcelFile():
    ...
    ws = wb.active
    ws.title = 'SNMP Location'
    ws['A2'] = nowDate
    ws['A4'] = 'Hostname'
    ws['B4'] = 'IP Address'
    ws['C4'] = 'SNMP Location'

    wb.save('SNMP_Location_Info.xlsx')
    wb.close()

def saveExcelFile(snmpLocation, elem, cellNumber):

    wb = openpyxl.load_workbook('SNMP_Location_Info.xlsx')
    ws = wb.active

    ws['A' + str(cellNumber)] = elem[0]
    ws['B' + str(cellNumber)] = elem[2]
    ws['C' + str(cellNumber)] = snmpLocation['snmpLocation']
